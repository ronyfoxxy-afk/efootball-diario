            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {sideItems.map(post => (
                <Link key={post.id} href={`/post/${post.slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ background: '#0e1014', border: '1px solid #1c1f26', borderRadius: 12, padding: '12px', display: 'flex', gap: 10, cursor: 'pointer', height: '100%' }}>
                    {post.cover_image && (
                      <div style={{ width: 60, height: 60, borderRadius: 8, background: `url(${post.cover_image}) center/cover`, flexShrink: 0 }} />
                    )}
                    <div style={{ minWidth: 0 }}>
                      {post.categories && (
                        <span style={{ background: post.categories.color + '18', color: post.categories.color, border: `1px solid ${post.categories.color}22`, fontSize: 9, fontWeight: 600, padding: '1px 6px', borderRadius: 4, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                          {post.categories.name}
                        </span>
                      )}
                      <p style={{ fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 700, color: '#e2e8f0', margin: '5px 0 3px', lineHeight: 1.25, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{post.title}</p>
        ...
        <Link href="/torneios" style={{ textDecoration: 'none', display: 'block', marginBottom: '1.5rem' }}>
          <div style={{ background: '#0e1014', border: '1px solid #1c1f26', borderLeft: '3px solid #e8b84b', borderRadius: 12, padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', cursor: 'pointer' }}>
            <div style={{ minWidth: 0 }}>
              ...
              <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: 17, fontWeight: 800, color: '#fff', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>🏆 Liga eFootball Diário</h3>
              <p style={{ fontSize: 12, color: '#4b5060' }}>Inscrição R$20 · Prêmio R$50 · Estilo Brasileirão</p>
            </div>
            <span style={{ fontSize: 13, color: '#8b909e', whiteSpace: 'nowrap', flexShrink: 0 }}>Ver →</span>
          </div>
        </Link>
