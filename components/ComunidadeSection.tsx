...
export default function ComunidadeSection() {
  return (
    <section style={{ marginBottom: '1.5rem' }}>
      <p style={{ fontSize: 11, color: '#4b5060', fontWeight: 600, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '0.875rem' }}>
        Siga nas redes
      </p>
      <div className="redes-grid">
        {LINKS.map(l => (
          <a key={l.label} href={l.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
            <div style={{ background: '#0e1014', border: '1px solid #1c1f26', borderRadius: 10, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <div style={{ width: 18, height: 18, color: l.color, flexShrink: 0 }} dangerouslySetInnerHTML={{ __html: l.svg }} />
              <span style={{ fontSize: 13, fontWeight: 500, color: '#8b909e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.label}</span>
            </div>
          </a>
        ))}
      </div>
